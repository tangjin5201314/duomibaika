...
//    CGContextSetFillColorWithColor(context, self.markColor.CGColor);
    CGContextSetRGBFillColor (context,  0, 0, 0, .5);//设置填充颜色

    CGContextAddRect(context, rect);
//    CGContextAddRect(context, self.targetView.frame);
...
- (void)reLocation{
    ...
    self.targetView.frame = anchorView.frame;
...
- (void)showTipView{
    self.popTipView = [[CHPopTipView alloc] initWithMessage:self.tooltip.messageRes attributedMessage:self.tooltip.attributedMessageRes];
