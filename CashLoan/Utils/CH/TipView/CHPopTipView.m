...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetRGBStrokeColor(c, 0.0, 0.0, 0.0, .8);	// black
    ...
    if (self.message) {
        ...
        [self.attributedMessage drawInRect:textFrame];
    }
}
...
- (void)presentPointingAtView:(UIView *)targetView inView:(UIView *)containerView animated:(BOOL)animated {
    ...
    if ( self.dismissTapAnywhere ) {
        ...
        [self.dismissTarget addTarget:self action:@selector(dismissWithTap) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (self.message!=nil) {
        ...
        textSize= [self.attributedMessage boundingRectWithSize:CGSizeMake(rectWidth, 99999.0) options:opts context:nil].size;
    }
    ...
    if (targetRelativeOrigin.y+targetView.bounds.size.height < containerRelativeOrigin.y) {
        ...
        if (sizeBelow > targetOriginInContainer.y) {
            pointerY = targetOriginInContainer.y + targetView.bounds.size.height/2 + targetView.bounds.size.height/2;
            pointDirection = PointDirectionUp;
        }
        else {
            pointerY = targetOriginInContainer.y + targetView.bounds.size.height/2 - targetView.bounds.size.height/2;
...
- (id)initWithMessage:(NSString *)messageToShow attributedMessage:(NSAttributedString *)attributedMessageRes {
    CGRect frame = CGRectZero;
    
    if ((self = [self initWithFrame:frame])) {
        self.message = messageToShow;
        self.attributedMessage = attributedMessageRes;
    }
    return self;
}
...
- (void)layoutWithTooltip:(Tooltip *)tooltip{
    if (tooltip) {
//        self.counter.hidden = [tooltip.flow getTooltipsCount] <= 1 ? YES : NO;
