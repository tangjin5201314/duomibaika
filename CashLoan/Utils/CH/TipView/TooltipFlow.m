...
@property (nonatomic, assign) BOOL needToRunWithDelay;
...
- (Tooltip *)getToolTipWithTooltipID:(NSString *)tooltipID{
    if (self.tooltips.count == 0) {
        return nil;
    }
    
    for (Tooltip *t in self.tooltips) {
        if ([t.anchorViewId isEqualToString:tooltipID]) {
            return t;
        }
    }
...
/*
 判断是否需要延迟运行
 */
- (BOOL)isNeedToRunWithDelay{
    return _needToRunWithDelay;
}

/*
 设置是否需要延迟运行
 */
- (void)setNeedToRunWithDelay:(BOOL)z{
    _needToRunWithDelay = z;
}
