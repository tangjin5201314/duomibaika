...
- (void)makeFlows{
    ...
    self.flows[@"load_detail"] = add(@"load_detail");
}

TooltipFlow * add(NSString *str){
    TooltipFlow *flow = [[TooltipFlow alloc] initWithFlowID:str];
    if ([@"load_detail" isEqualToString:str]) {
        [flow addTooltip:[[Tooltip alloc] initWithFlow:flow res:@"我们会与还款日当日向您尾号258的银行卡进行自动扣款，请您保持还款日当日卡内余额充足。" anchorViewId:@"load_detail_help"]];
    }
    return flow;
}
...
- (void)cofigLoanTooptip{
    ...
    UIColor *textColor = [UIColor colorWithRed:0x66/255.0 green:0x66/255.0 blue:0x66/255.0 alpha:1];
    UIColor *textColor2 = [UIColor colorWithRed:0x33/255.0 green:0x33/255.0 blue:0x33/255.0 alpha:1];
    
    NSAttributedString *str1 = [[NSAttributedString alloc] initWithString:@"我们会与还款日当日向您" attributes:@{NSFontAttributeName: font, NSParagraphStyleAttributeName:textStyle, NSForegroundColorAttributeName:textColor}];
    NSAttributedString *str2 = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"尾号%@", self.cardNo] attributes:@{NSFontAttributeName: font, NSParagraphStyleAttributeName:textStyle, NSForegroundColorAttributeName:textColor2}];
    NSAttributedString *str3 = [[NSAttributedString alloc] initWithString:@"的银行卡进行自动扣款，请您保持还款日当日卡内余额充足。" attributes:@{NSFontAttributeName: font, NSParagraphStyleAttributeName:textStyle, NSForegroundColorAttributeName:textColor}];
    [str appendAttributedString:str1];
    [str appendAttributedString:str2];
    [str appendAttributedString:str3];
    
    [self setMessage:nil attributedMessage:str forTooltip:@"load_detail_help" inFlow:@"load_detail"];
}

- (void)startFlowIfNeeded:(UIViewController *)activity flowID:(NSString *)flowID{
    
    if ([flowID isEqualToString:@"load_detail"]) {
        [self cofigLoanTooptip];
    }
...
- (void)setMessage:(NSString *)message attributedMessage:(NSAttributedString *)attributedString forTooltip:(NSString *)tooltipID inFlow:(NSString *)flowID{
    Tooltip * tooltip = [self.flows[flowID] getToolTipWithTooltipID:tooltipID];
    tooltip.messageRes = message;
    tooltip.attributedMessageRes = attributedString;
}
...
//    ((BaseBaseActivity) activity).disableInteraction();
    if (self.tooltipLayoutContainer == nil) {
        ...
        UIView *containerSuperView = activity.view;
        if (activity.navigationController) {
            containerSuperView = activity.navigationController.view;
        }
        
        [containerSuperView addSubview:self.tooltipLayoutContainer];
...
- (void)resetFlow:(NSString *)flowID{
    [self.userDefaults setBool:NO forKey:getKeyForFlowShown(flowID)];
    [self.userDefaults setObject:@"-1" forKey:getKeyForTooltipIndex(flowID)];
    [self.userDefaults synchronize];
}
