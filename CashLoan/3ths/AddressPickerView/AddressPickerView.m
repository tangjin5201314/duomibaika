...
- (UIButton *)cancelBtn{
    if (!_cancelBtn) {
        ...
        [_cancelBtn setTitleColor:[UIColor tabBarColor] forState:UIControlStateNormal];
...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        ...
        [_sureBtn setTitle:@"完成"
                  forState:UIControlStateNormal];
        [_sureBtn setTitleColor:[UIColor tabBarColor] forState:UIControlStateNormal];
...
- (void)loadPickerView{
    ...
    [self.addressPickerView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(@(TITLEHEIGHT));
        make.left.mas_equalTo(@0);
        make.right.mas_equalTo(@0);
...
- (void)showOrHide:(BOOL)isShow{
    ...
    CGRect fromF = toF;
    fromF.origin.y += CGRectGetHeight(self.frame);
    
    toFrame = isShow ? toF : fromF;
    fromFrame = isShow ? fromF : toF;
    
    snapshotView.frame = fromFrame;
    [self.markView addSubview:snapshotView];

    self.hidden = YES;
    self.markView.alpha = fromAlpha;

    [UIView animateWithDuration:0.3 animations:^{
        snapshotView.frame = toFrame;
        self.markView.alpha = toAlpha;
...
- (void)sureBtnClicked{
    if ([_delegate respondsToSelector:@selector(addressPickerView:sureBtnClickReturnProvince:City:Area:)]) {
        ...
        [_delegate addressPickerView:self sureBtnClickReturnProvince:p.name City:c.cityName Area:c.areas[selectArea]];
    }
}
