...
static char const * const kContentInsetKey =       "kContentInsetKey";
...
- (UIEdgeInsets)gzw_contentInset{
    NSValue *value = objc_getAssociatedObject(self, kContentInsetKey);
    return [value UIEdgeInsetsValue];
}


- (void)setGzw_contentInset:(UIEdgeInsets)gzw_contentInset{
    objc_setAssociatedObject(self, kContentInsetKey, [NSValue valueWithUIEdgeInsets:gzw_contentInset], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if ([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0)
    {
        ...
        view.frame = CGRectMake(0, -self.gzw_contentInset.top, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame)+self.gzw_contentInset.top);
...
- (UIButton *)button
{
    if (!_button)
    {
        ...
        [_button setTitleColor:[UIColor tabBarColor] forState:UIControlStateNormal];
...
- (void)setupConstraints
{
    ...
    if (_customView) {
        ...
        CGFloat verticalSpace = self.verticalSpace ? : 11; // Default is 11 pts
