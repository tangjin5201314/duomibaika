...
@interface UICollectionView (GzwByCH)<DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
/**
 *  是否在加载 YES:转菊花 or NO:立即空状态界面
 *  PS:在加载数据前设置为YES(必需)，随后根据数据调整为NO(可选)
 */
@property (nonatomic, assign)BOOL showLoading;  //菊花
...
@property (nonatomic, copy)NSAttributedString *descriptionAttributedText;// 空状态下的文字详情
