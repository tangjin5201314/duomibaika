...
static const BOOL showloadingKey;
...
- (void)setShowLoading:(BOOL)showLoading{
    objc_setAssociatedObject(self, &showloadingKey, @(showLoading), OBJC_ASSOCIATION_ASSIGN);
...
- (BOOL)showLoading{
    id tmp = objc_getAssociatedObject(self, &showloadingKey);
    NSNumber *number = tmp;
...
- (NSAttributedString *)buttonTitleForEmptyDataSet:(UIScrollView *)scrollView forState:(UIControlState)state
{
    if (self.loading || !self.buttonText) {
        ...
        // 某种状态下的颜色
        UIColor *colorOne = [UIColor tabBarColor];
        UIColor *colorTow = [UIColor tabBarColor];
        // 判断外部是否有设置
